"""
ARB TRIANGULAR BOT v5 - CORRECOES DEFINITIVAS:
  1. Scan loop: fetch_ticker removido - usava auth desnecessaria que falhava
  2. Saldo real: usa /api/v3/account via HMAC directo (nao geo-restrito)
     O ccxt fetch_balance chamava /sapi/v1/capital/config/getall (restrito EUA)
  3. Saldo simulacao: separado do real
  4. Logs a cada 5 scans para confirmar operacao
  5. Erro capturado com traceback completo no console Render
"""
...
# ── Saldo real via REST directo ───────────────────────────────────────────────
# Usa /api/v3/account (endpoint global, nao geo-restrito).
# fetch_balance() do ccxt chama /sapi/v1/capital/config/getall que e restrito
# nos EUA onde o Render esta. Esta funcao evita esse problema completamente.
def get_saldo_real(api_key, api_secret):
    try:
        ...
        url     = "https://api.binance.com/api/v3/account?" + params + "&signature=" + sig
        ...
        add_log("Saldo Binance: " + msg, "warn")
        return None
    except Exception as e:
        add_log("Saldo REST: " + str(e)[:60], "warn")
        return None

# ── Teste de conexao publica ──────────────────────────────────────────────────
def testar_binance():
    try:
        r    = req.get("https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT", timeout=8)
        data = r.json()
        p    = float(data.get("price", 0))
        add_log("Binance conectada | BTC=" + "${:,.2f}".format(p), "success")
        return True
    except Exception as e:
        add_log("Falha conexao: " + str(e)[:60], "error")
        return False
...
def preco_ob(par, lado, usdt):
    try:
        ...
        for p, q in ns:
            ...
            custo += v; acum += v
        if acum < usdt * 0.95: return None, None, liq
        med  = custo / (acum / best)
...
def executar_arb(res):
    ...
    try:
        ...
        add_log("Saldo insuficiente para executar arb", "error")
        return False, 0
    except Exception as e:
...
def bot_loop():
    ...
    # 1. Teste de conexao (endpoint publico — sem auth, sem restricao geo)
    if not testar_binance():
        BOT["running"] = False
        return

    # 2. Inicializar ccxt (para fetch_order_book que e publico)
    ex = ccxt.binance({
        "apiKey":          BOT["api_key"],
    ...
    if not BOT["paper"]:
        if not BOT["api_key"] or not BOT["api_secret"]:
            add_log("Modo REAL requer chaves API — configura em Config!", "error")
            BOT["running"] = False
            return
        usdt = get_saldo_real(BOT["api_key"], BOT["api_secret"])
        if usdt is not None:
            BOT["saldo_conta"] = usdt
            add_log("Saldo real Binance: $" + "%.4f" % usdt + " USDT", "info")
        else:
            BOT["saldo_conta"] = 0.0
            add_log("Nao foi possivel ler saldo real — verifica as chaves API", "warn")
    else:
        # Simulacao: saldo virtual = capital inicial
        BOT["saldo_conta"] = BOT["capital"]
        add_log("Modo simulacao | Saldo virtual: $" + "%.4f" % BOT["saldo_conta"] + " USDT", "info")

    add_log("Scan activo | " + str(len(TRIANGULOS)) + " triangulos | intervalo 2s", "info")
    ...
    while BOT["running"]:
        try:
            ...
            if BOT["cap_inicial"] > 0:
                ...
                if dd >= BOT["max_dd"]:
                    add_log("DRAWDOWN " + "%.2f" % dd + "% >= " + str(BOT["max_dd"]) + "% — Bot parado!", "error")
                    BOT["running"] = False
                    break
            ...
            # Sync saldo real cada 2 min (via REST directo)
            ...
            if BOT["scans"] % 5 == 0:
                if scan_todos:
                    ...
                    add_log(
                        ...
                        " | Melhor: " + top["label"] +
                        " " + ("%+.4f" % top["pct"]) + "%" +
                        " | " + str(len(scan_todos)) + " pares" +
                        " | " + str(len(ops)) + " validos",
                        "info"
                    )
                else:
                    add_log("Scan #" + str(BOT["scans"]) + " | Sem dados do orderbook!", "warn")
            ...
            add_log("Rede: " + str(e)[:60], "warn")
            time.sleep(15)
        except ccxt.RateLimitExceeded:
            add_log("Rate limit Binance", "warn")
            time.sleep(30)
        except Exception as e:
...
<div class="pg show" id="pgDash">
  ...
  <div id="ag">
    <div class="ab abm" id="bxE"><div class="an" id="nE" style="font-size:36px;color:var(--gr)">0</div><div class="al" style="color:var(--gr)">Executadas</div></div>
...
<div class="pg" id="pgConfig">
  ...
  <div class="cs">
    ...
    <div class="ib">Na Binance activa <strong style="color:var(--gr)">Spot Trading</strong> apenas &#8212; <strong style="color:var(--rd)">NUNCA Withdrawals</strong></div>
  </div>
  <div class="cs">
  ...
  <div class="cs">
    ...
    <div class="trow">
      <div><h4>Simulacao (Paper Trading)</h4><p>Dados reais Binance &#8212; sem ordens reais</p></div>
      <button class="tgl" id="tp" onclick="tgP()" style="background:var(--gr)"><div class="tk" id="tpk" style="left:24px"></div></button>
    </div>
    <div id="wr" style="display:none" class="wb">MODO REAL activo &#8212; o bot vai executar ordens reais! Com $0.93 USDT o capital e insuficiente (minimo: $10 USDT).</div>
  </div>
  <div class="cs">
    ...
    <div class="fr"><label class="fl">Gatilho de reinvestimento</label>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px" id="gb">
        ...
        <button class="qb agd" onclick="sG(this,10)" data-v="10">10%</button>
...
function poll(){
  fetch('/api/status').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('subc').textContent=d.paper?'simulado \xb7 actualiza a cada arb':'saldo real Binance';
    ...
    document.getElementById('subb').textContent=d.running?(Math.abs(d.roi).toFixed(4)+'% ROI \xb7 apos reinvestimento'):'apos reinvestimento';
    // Flash conta
    ...
    // Mode badge
    const mlEl=document.getElementById('ml');mlEl.innerHTML=d.running?(d.paper?'<span class="tag tor">Simulacao \xb7 Dados Reais Binance</span>':'<span class="tag trd">Modo Real \xb7 Ordens Reais</span>'):'';
