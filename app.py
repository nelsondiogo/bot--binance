...
TRIANGULOS = [
    # BTC como moeda intermediaria
    ["USDT","BTC","ETH"],  ["USDT","BTC","BNB"],  ["USDT","BTC","SOL"],
    ["USDT","BTC","XRP"],  ["USDT","BTC","ADA"],  ["USDT","BTC","DOGE"],
    ["USDT","BTC","AVAX"], ["USDT","BTC","LINK"], ["USDT","BTC","DOT"],
    ["USDT","BTC","POL"],  # POL = antigo MATIC (renomeado Set 2024)
    # ETH como moeda intermediaria
    ["USDT","ETH","BNB"],  ["USDT","ETH","SOL"],  ["USDT","ETH","XRP"],
    ["USDT","ETH","DOGE"], ["USDT","ETH","AVAX"], ["USDT","ETH","LINK"],
    # BNB como moeda intermediaria
    ["USDT","BNB","SOL"],  ["USDT","BNB","XRP"],
    ["USDT","BNB","DOT"],  ["USDT","BNB","ADA"],
]
...
def get_saldo_usdt(api_key, api_secret):
    ...
    Endpoint /api/v3/account - global, sem restricao geografica.

    Se aparecer "Invalid API-key, IP, or permissions":
    Na Binance -> Gestao de API -> desactiva "Restrict to Trusted IPs" -> Guarda
    """
    ...
    if data and "balances" in data:
        for b in data["balances"]:
            if b["asset"] == "USDT":
                ...
                return usdt
        add_log("USDT nao encontrado nos balances Spot", "warn")
        return 0.0
    if data:
        code = data.get("code", 0)
        msg  = data.get("msg", str(data))[:100]
        if code in (-2014, -2015) or "Invalid API" in str(msg):
            add_log(
                "Chave API invalida ou IP bloqueado. "
                "Binance -> Gestao API -> desactiva 'Restrict to Trusted IPs'",
                "error"
            )
        else:
