"""
ARB TRIANGULAR BOT - BYBIT v1
==============================
Migrado da Binance para Bybit:
  - Sem bloqueios geograficos
  - API publica do orderbook sem restricoes
  - Saldo via /v5/account/wallet-balance (HMAC SHA256)
  - Pares verificados no Bybit Spot
  - Formato de simbolo: BTCUSDT (igual a Binance)
  - Frankfurt (EU) no Render - sem restricoes
"""
...
SESSION = req.Session()
SESSION.headers.update({"User-Agent": "Mozilla/5.0 ARB-Bot/1.0"})
...
# Taxa Bybit Spot: 0.1% maker/taker (sem VIP)
# Com 3 legs: 1 - (1-0.001)^3 = ~0.2997% custo total
TAXA = 0.001

# Triângulos verificados no Bybit Spot
# Todos os pares intermédios existem no Bybit
# Bybit Spot: apenas BTC como bridge - pares confirmados
# ETH/SOL, DOGE/ETH, SOL/ETH etc NAO existem no Bybit Spot
# So ALTUSDT e ALTBTC existem com volume suficiente
TRIANGULOS = [
    ["USDT","BTC","ETH"],   # BTCUSDT + ETHBTC  + ETHUSDT
    ["USDT","BTC","XRP"],   # BTCUSDT + XRPBTC  + XRPUSDT
    ["USDT","BTC","ADA"],   # BTCUSDT + ADABTC  + ADAUSDT
    ["USDT","BTC","LTC"],   # BTCUSDT + LTCBTC  + LTCUSDT
    ...
    ["USDT","BTC","EOS"],   # BTCUSDT + EOSBTC  + EOSUSDT
    ["USDT","BTC","ETC"],   # BTCUSDT + ETCBTC  + ETCUSDT
    ["USDT","BTC","ATOM"],  # BTCUSDT + ATOMBTC + ATOMUSDT
]
...
def bybit_get_public(path, params=None):
    """Chamada publica ao Bybit - sem auth, sem restricoes geo."""
    try:
        r = SESSION.get(BYBIT_BASE + path, params=params, timeout=6)
        if r.status_code == 200:
            return r.json()
        add_log("Bybit HTTP " + str(r.status_code) + ": " + r.text[:60], "warn")
        return None
    except Exception as e:
        add_log("REST public: " + str(e)[:60], "warn")
        return None

def bybit_get_private(path, api_key, api_secret, params=None):
    """
    Chamada privada ao Bybit com assinatura HMAC SHA256.
    Bybit v5: timestamp + api_key + recv_window + query_string
    """
    try:
        ...
        sign_str   = ts + api_key + recv_win + qs
        sig        = hmac.new(
        ...
        add_log("REST private: " + str(e)[:60], "warn")
        return None

def get_preco_btc():
    """Teste de conectividade via preco BTC."""
    data = bybit_get_public("/v5/market/tickers", {"category": "spot", "symbol": "BTCUSDT"})
    if data and data.get("retCode") == 0:
        try:
            p = float(data["result"]["list"][0]["lastPrice"])
            return p
        except:
            pass
    return None
...
def get_saldo_usdt(api_key, api_secret):
    """
    Le saldo USDT da conta Unified ou Spot da Bybit.
    Bybit nao requer whitelist de IPs por defeito.
    Permissoes necessarias na chave: Read + Spot Trading
    """
    # Tenta Unified account primeiro (mais comum)
    data = bybit_get_private(
        ...
        try:
            ...
            for c in coins:
                if c["coin"] == "USDT":
                    ...
                    add_log("Saldo Unified lido: $" + "%.4f" % usdt + " USDT", "success")
                    return usdt
        except:
            pass

    # Fallback: Spot account
    data2 = bybit_get_private(
        "/v5/account/wallet-balance",
        api_key, api_secret,
        {"accountType": "SPOT"}
    )
    if data2 and data2.get("retCode") == 0:
        try:
            coins = data2["result"]["list"][0]["coin"]
            for c in coins:
                if c["coin"] == "USDT":
                    usdt = float(c.get("availableToWithdraw") or c.get("walletBalance") or 0)
                    add_log("Saldo Spot lido: $" + "%.4f" % usdt + " USDT", "success")
                    return usdt
        except:
            pass

    # Erro
    code = (data or {}).get("retCode", "?")
    msg  = (data or {}).get("retMsg", "sem resposta")[:80]
    if code in (10003, 10004, 33004):
        add_log(
            "Chave API invalida. Na Bybit: API Management -> "
            "certifica que tem permissoes 'Read' e 'Spot Trading'",
            "error"
        )
    else:
        add_log("Saldo erro (code " + str(code) + "): " + str(msg), "warn")
    return None
...
def get_orderbook_bybit(symbol, limit=20):
    """
    Orderbook via /v5/market/orderbook - publico, sem auth.
    symbol: ex "BTCUSDT"
    Retorna {"b": [[price,qty]...], "a": [[price,qty]...]}
    """
    data = bybit_get_public(
        "/v5/market/orderbook",
    ...
    if data:
        ...
        msg  = data.get("retMsg", "")[:40]
        if code != 0:
            # Log apenas 1 em cada 50 erros para nao encher logs
            if BOT["scans"] % 50 == 1:
                add_log("OB falhou: " + symbol + " (code " + str(code) + " " + msg + ")", "warn")
    return None

def preco_ob(symbol_slash, lado, usdt):
    """
    Calcula preco medio de execucao do orderbook.
    symbol_slash: "BTC/USDT" -> converte para "BTCUSDT"
    lado: "c" = comprar (asks), "v" = vender (bids)
    """
    symbol = symbol_slash.replace("/", "")
    ob = get_orderbook_bybit(symbol, limit=20)
    if ob is None:
...
def calcular(tri, capital):
    ...
    try:
        # Leg 1: USDT -> A
        p1, s1, l1 = preco_ob(A + "/" + base, "c", capital)
        if p1 is None: return None
        qa = (capital / p1) * (1 - TAXA)

        # Leg 2: A -> B
        p2, s2, l2 = preco_ob(B + "/" + A, "c", qa * p1 * (1 - TAXA))
        if p2 is None: return None
        qb = (qa / p2) * (1 - TAXA)

        # Leg 3: B -> USDT
        p3, s3, l3 = preco_ob(B + "/" + base, "v", qb * p2 * p1 * (1 - TAXA))
...
def executar_arb(res):
    ...
    # MODO REAL via ccxt Bybit
    global ex
    if ex is None:
        add_log("ccxt nao inicializado", "error")
        return False, 0
    ...
    try:
        t0 = time.time()
        # Bybit: quantidade em unidades base
        q1 = res["capital"] / p1
        o1 = ex.create_market_order(par1, "buy",  round(q1, 6))
        time.sleep(0.1)
        q2 = float(o1.get("filled", qa)) / p2
        o2 = ex.create_market_order(par2, "buy",  round(q2, 6))
        time.sleep(0.1)
        q3 = float(o2.get("filled", qb))
        o3 = ex.create_market_order(par3, "sell", round(q3, 6))
...
def bot_loop():
    ...
    add_log("Bot iniciado | $" + str(BOT["capital"]) + " | JC " + str(BOT["gatilho_jc"]) + "% | " + modo + " | BYBIT", "success")

    # 1. Teste de conectividade
    preco = get_preco_btc()
    if preco is None:
        add_log("Falha ao conectar ao Bybit. Verifica a rede do servidor.", "error")
        BOT["running"] = False
        return
    BOT["btc_preco"] = preco
    add_log("Bybit conectado | BTC/USDT = $" + "{:,.2f}".format(preco), "success")

    # 2. Saldo
    if not BOT["paper"]:
        if not BOT["api_key"] or not BOT["api_secret"]:
            add_log("Modo REAL requer chaves API Bybit — configura em Config!", "error")
            BOT["running"] = False
            return
        # ccxt para ordens reais
        ex = ccxt.bybit({
        ...
        if usdt is not None:
            ...
            if usdt < 1:
                add_log("Saldo muito baixo ($" + "%.4f" % usdt + "). Minimo recomendado: $10 USDT", "warn")
        else:
            BOT["saldo_conta"] = 0.0
    else:
        BOT["saldo_conta"] = BOT["capital"]
        add_log("Simulacao | Saldo virtual: $" + "%.4f" % BOT["saldo_conta"] + " USDT", "info")
        add_log("Orderbook real Bybit ao vivo", "info")

    add_log("Scan activo | " + str(len(TRIANGULOS)) + " triangulos | 2s/ciclo", "info")
    ...
    while BOT["running"]:
        try:
            ...
            if BOT["cap_inicial"] > 0:
                ...
                if dd >= BOT["max_dd"]:
                    add_log("DRAWDOWN " + "%.2f" % dd + "% — Bot parado!", "error")
                    BOT["running"] = False; break

            h = datetime.now().hour
            if h != BOT["hora_atual"]:
                BOT["hora_atual"] = h; BOT["arbs_hora"] = 0
            if BOT["arbs_hora"] >= 20:
            ...
            if not BOT["paper"] and time.time() - ult_saldo > 120:
                usdt = get_saldo_usdt(BOT["api_key"], BOT["api_secret"])
                if usdt is not None: BOT["saldo_conta"] = usdt
                ult_saldo = time.time()

            # ── SCAN ──────────────────────────────────────
            BOT["scans"] += 1
            scan_todos = []; ops = []; erros = 0
            ...
            for tri in TRIANGULOS:
                ...
                if res is None: erros += 1; continue
            ...
            if BOT["scans"] % 5 == 0:
                ...
                if n > 0:
                    ...
                    add_log(
                        "Scan #" + str(BOT["scans"]) +
                        " | " + str(n) + "/" + str(len(TRIANGULOS)) + " pares" +
                        " | Melhor: " + top["label"] +
                        " " + ("%+.4f" % top["pct"]) + "%" +
                        " | " + str(len(ops)) + " validos",
                        "info"
                    ...
                    add_log(
                        "Scan #" + str(BOT["scans"]) +
                        " | 0 pares — " + str(erros) + " erros de orderbook",
                        "warn"
                    )
            ...
            if ops:
                ...
                if check and check["ok"] and abs(check["pct"] - melhor["pct"]) < 0.3:
                    ...
                    if ok:
                        ...
                        BOT["arbs_exec"]  += 1; BOT["arbs_hora"] += 1
                        BOT["last_arb"]    = melhor
...
@app.route("/api/logs")
def api_logs():    return jsonify(BOT["logs"][:100])

@app.route("/api/scan")
def api_scan():    return jsonify(BOT["scan_data"][:20])
...
def api_start():
    ...
    with _lock:
        BOT.update({
            ...
            "cooldowns": {}, "logs": [], "scan_data": [],
            "last_arb": None, "marcos": [], "running": True, "btc_preco": 0.0,
        })
...
.logo{font-family:'Syne',sans-serif;font-weight:800;font-size:17px;letter-spacing:2px;background:linear-gradient(135deg,var(--cy),var(--gr));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
<div id="app">
<div id="top">
  <div class="logo">ARB <span style="-webkit-text-fill-color:var(--cy)">&#9651;</span><small>BYBIT</small></div>
  <div id="tr">
    <span id="ts"></span>
...
<div class="pg show" id="pgDash">
  <div id="ds">
    <div class="scc" id="sca">
      <div class="slb"><span class="ld" id="ldt"></span>Conta Bybit<span class="tag tcy" id="ltg" style="display:none;font-size:8px">LIVE</span></div>
...
<div class="pg" id="pgConfig">
  ...
  <div class="cs">
    <div style="margin-bottom:14px"><span class="tag tcy">Chaves API Bybit</span></div>
    <div class="fr"><label class="fl">API Key</label><input class="inp" type="text" id="ck" placeholder="Cole a tua API Key Bybit" autocomplete="off" autocorrect="off" autocapitalize="none" spellcheck="false"></div>
    <div class="fr"><label class="fl">API Secret</label><input class="inp" type="password" id="cs" placeholder="Cole o teu Secret Key Bybit" autocomplete="off" autocorrect="off" autocapitalize="none" spellcheck="false">
    ...
    <div class="ib">
      Na Bybit: <strong style="color:var(--gr)">API Management</strong> &#8594; Create New Key<br>
      Permissoes: <strong style="color:var(--cy)">Read</strong> + <strong style="color:var(--cy)">Spot Trading</strong><br>
      <strong style="color:var(--rd)">NAO actives Withdrawals</strong><br>
      IP Restriction: <strong style="color:var(--gr)">deixa em branco</strong> (sem restricao de IP)
    </div>
  </div>
  <div class="cs">
    ...
    <div class="fr"><label class="fl">Saldo total da conta Bybit (USDT)</label><input class="inp" type="number" id="cco" value="13" min="0" step="0.01" oninput="document.getElementById('mx').textContent='$'+this.value;upG()"></div>
  </div>
  <div class="cs">
    <div style="margin-bottom:14px"><span class="tag tgr">Modo de Operacao</span></div>
    <div class="trow">
      <div><h4>Simulacao (Paper Trading)</h4><p>Orderbook real Bybit &#183; sem ordens reais</p></div>
      <button class="tgl" id="tp" onclick="tgP()" style="background:var(--gr)"><div class="tk" id="tpk" style="left:24px"></div></button>
    </div>
    <div id="wr" style="display:none" class="wb">MODO REAL activo &#8212; o bot vai executar ordens reais na Bybit!</div>
  </div>
  <div class="cs">
...
function poll(){
  fetch('/api/status').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('subc').textContent=d.paper?'simulado \xb7 arb virtual':'saldo real Bybit Spot';
    ...
    const mlEl=document.getElementById('ml');mlEl.innerHTML=d.running?(d.paper?'<span class="tag tor">Simulacao \xb7 Orderbook Real Bybit</span>':'<span class="tag trd">Modo Real \xb7 Bybit Spot</span>'):'';
